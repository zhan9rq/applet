			...
			<scroll-view scroll-y="true" class="right">
				<view v-for="(fItem, fIndex) in categoryList" :key="fItem.id">
					<view v-if="fIndex === showCategoryIndex" class="category">
						<view v-if="cateTop && cateTop.cover" class="banner" @tap="indexTopToDetailPage(cateTop)">
							<image :src="cateTop && cateTop.cover" mode="aspectFill" />
						</view>
						<view class="box" v-for="sItem in fItem.child" :key="sItem.id">
							<view class="second-category-text" @tap="navTo(`/pages/product/list?cate_id=${sItem.id}`)">
								{{ sItem.title }}11{{sItem.id}}
								<text class="iconfont iconyou"></text>
							</view>
							<view class="list" v-if="sItem.child && sItem.child.length > 0">
								<view class="box" v-for="tItem in sItem.child" :key="tItem.id"
